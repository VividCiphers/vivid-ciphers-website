; (function () {
    ...
    function toggleTransition() {
      hamburger.classList.toggle('transition-top');
    };

    (window.innerWidth <= 600) ? alignLeft() : false;

    window.addEventListener('resize', function(){
      (window.innerWidth > 600 && hamburger.classList.contains('transition-top')) ? hamburger.classList.toggle('transition-top') : false;
    });
